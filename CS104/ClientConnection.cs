...
namespace lib60870.CS104
{
    ...
    public class ClientConnection : IMasterConnection
    {
        ...
        private readonly object _lockReceivedASDUs = new object();
        ...
        private void ReceivedASDUsEnqueue(ASDU asdu)
        {
            lock (_lockReceivedASDUs){
            ...
            while (callbackThreadRunning)
            {
                try
                {
                    while ((receivedASDUs.Count > 0) && (callbackThreadRunning) && (running))
                    {
                        ...
                        if (ReceivedASDUsTryDequeue(out asdu))
                        {
                                HandleASDU(asdu);
                        }
                    }
